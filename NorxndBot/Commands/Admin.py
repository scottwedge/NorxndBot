...
# Send announcement to channel
    ...
    async def ann(self, ctx, args):
        ...
        await channel.send(f"ANNOUNCEMENT TEXT LIKE @everyone New announcement + {args}") # <- Args is text that you type into an command like ?Announcement "Text"
...
# Deletes number of messages
    ...
    async def clean(self, ctx, limit: int): # Limit is how much messages you want to delete
        limit = limit + 1
...
# NorxndBot warn systen
    ...
    async def Warn(self, ctx, args1: discord.Member, args2="no reason"):
        ...
        WarnTree = ctx.guild.get_role(000000000000000000) # In this 0 enter 3th Warning role ID
        ...
        for roles in Member.roles: # Check all roles that member have
            if roles != WarnOne: # If checks if Member's role isnt 1st Warning
                await Member.add_roles(WarnOne) # Add 1st Warning role
            if roles == WarnOne: # If checks if Member's role is 1st Warning
                await Member.add_roles(WarnTwo) # Add 2nd Warning role
            if roles == WarnTwo: # If checks if Member's role is 2nd Warning
                await Member.add_roles(WarnTree) # Add 3th Warning role
            ...
            if roles == WarnTree:
                ...
                Action = "banned" # Otional
                await ctx.guild.ban(Member, reason=args2) # Ban Member

        channel = await Member.create_dm() # Remeber sending messages to specyfic channels? This is for sending DMS, and you use it the same as await channel.send()
        ...
        await ctx.message.delete()
        await ctx.send("Warned {}".format(args1), delete_after=3) # Optional. ctx.send() send message to channel where trigger command was sended
        LOG=("MESSAGE TO LOGS") # You can use datetime.datetime.now() in format to add time to your logs. You can use datetime.datetime.utcnow() to add time in UTC
        print(LOG)
        self.bot.BOT_LOG.append(LOG)
...
# Send alert to User
    ...
    @commands.has_any_role("ADMIN / MODERATOR ROLE NAME")
    async def Alert(self, ctx, args1: discord.Member, args2="This must be mistake"): # self,ctx,args1 etc. Is command arguments. You MUST have ctx. You must have self if command isnt in main bot file.
        Member = args1
        ...
        LOG=("TEXT TO LOG") # For log I recommend f"{ctx.author} used COMMAND NAME command with arguments: {ARGUMENT 1} and {ARGUMENT 2} and {ARGUMENT 3} (If you only have ctx and self you dont must write with arguments..., if you got only one add on argument you dont must to type and) at {datetime.datetime.utcnow()}"
        print(LOG)
        self.bot.BOT_LOG.append(LOG)
...
# Send custom DM to user
    ...
    async def SendDM(self, ctx, Member: discord.Member, text):
        ...
        await channel.send(text)
        await ctx.message.delete()
        await ctx.send("DM sended to {}".format(Member), delete_after=3)
        LOG=("LOG TEXT")
        print(LOG)
        self.bot.BOT_LOG.append(LOG)
